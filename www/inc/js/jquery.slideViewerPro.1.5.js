    ...
    .wrap(jQuery("<div style='width:"+ pictWidth +"px; overflow: hidden; position: relative; top: 0; left: 0'>"));
		...
		container.each(function(i) {
			...
			jQuery(this).find("li").each(function(n) { 
						jQuery("div#thumbSlider" + j + " ul").append("<li><a title='" + jQuery(this).find("img").attr("alt") + "' href='#' target='_blank'><img class='ui-img' width='"+ thumbsWidth +"' heights='"+ thumbsHeight +"' src='" + jQuery(this).find("img").attr("src2") + "' /><p class='tmbrdr'>&nbsp;<\/p><\/a><\/li>");						
			});
			...
			jQuery("div#thumbSlider" + j + " a").each(function(z) {			
				jQuery(this).bind("click", function(){
					...
					//edit by wlf 2012-3-25 原为return false;
			...
			jQuery("div#thumbSlider" + j).css({position: "relative", left: contBorder, top: settings.thumbsTopMargin+"px", width: settings.thumbs*thumbsWidth+((settings.thumbsRightMargin*settings.thumbs)-settings.thumbsRightMargin), /* heights: thumbsHeight,*/ textAlign: "center", overflow: "hidden", margin: "0 auto"});
			...
			(jQuery("div#thumbSlider" + j).width()+(settings.buttonsWidth*2) >= pictWidth)? ("ALERT: THE THUMBNAILS SLIDER IS TOO WIDE! \nthumbsPercentReduction and/or buttonsWidth needs to be scaled down!") : null;
		j++;
  });	
};