// 修改宽度 替换 _sliderWidth,将 _halfWidth 替换成想要的
var _imgArray = [], _sliderWidth = 1004, _halfWidth = 502,worksprites = new Image;
worksprites.src = 'images/works-sprites.png';
...
function funcmaincontent(){
	...
	function startslide(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			var _pos = Math.round(_sliderWidth*(i-_currentpage)+$(window).width()/2-_halfWidth);
			var _opa = 0.5;
			if(i == _currentpage)_opa = 1;
			if(_pos > $(window).width()){
				_pos -= _maxpage*_sliderWidth
			}else if(_pos < -_sliderWidth){
				_pos += _maxpage*_sliderWidth
			}
	...
	function nextPage(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			...
			if(_pos > $(window).width()){
				_pos -= _maxpage*_sliderWidth
			}else if(_pos < -_sliderWidth*2){
				_pos += _maxpage*_sliderWidth
			}
			_pict.eq(i)
			.stop()
			.css({
				left:_pos+_sliderWidth
			})
			.animate({
				left:_pos,
				opacity:_opa
	...
	function prevPage(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			...
			var _opa = 0.5;
			if(i == _currentpage)_opa = 1;
			if(_pos < -_sliderWidth){
				_pos += _maxpage*_sliderWidth
			}else if(_pos > $(window).width()+_sliderWidth){
				_pos -= _maxpage*_sliderWidth
			}
